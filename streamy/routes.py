...
def start_social(configuration_file):
    if not os.path.isfile(configuration_file):
        raise IOError("Config file not found {0}".format(configuration_file))
    ...
    for name, account_config in config:
        if account_config['site_type'] == 'twitter':
            accounts.append(Twitter(name, configuration_file))
        elif account_config['site_type'] == 'statusnet':
            accounts.append(StatusNet(name, configuration_file))
        else:
            self.logger.warning("Unknown account type %s, skipping" % name)

    GLOBAL_CONTEXT['accounts'] = accounts
    for account in GLOBAL_CONTEXT['accounts']:
        account.feed.fetch()
...
def runserver(host='', port='', debug=None, user='', group='',
              settings_file='', version=False, configuration_file='config.json'):

    if version:
        print 'Streamy V%s' % settings.VERSION
        sys.exit(0)
    ...
    try:
        _, app = get_app(debug, settings_file, settings=settings)
    ...
    thread.start_new_thread(start_social, (configuration_file))
